#!/usr/bin/env python3
"""Run websocket ingestion cycles and persist raw market/order/fill stream payloads."""
...
def _build_broker_config(
    ...
    *,
    ...
    return {
        ...
        "order_ledger_path": execution_cfg.get(
            "order_ledger_path", "data/analytics/order_ledger.jsonl"
        ),
        "exchanges": {},
...
async def _run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if "initial_capital" not in risk_cfg:
        raise ValueError("risk.initial_capital must be set in config before ws ingestion runs.")
    ...
    report_path = out_dir / f"ws_ingestion_{stamp}.json"
